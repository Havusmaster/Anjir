...
const FILL = new Set<string>(['github', 'linkedin', 'telegram', 'play', 'apple'])
...
@Component({
  ...
  template: `
    <svg
      viewBox="0 0 24 24"
      aria-hidden="true"
      [attr.fill]="FILL.has(name) ? 'currentColor' : 'none'"
      [attr.stroke]="FILL.has(name) ? 'none' : 'currentColor'"
      [attr.stroke-width]="STROKE_WIDTH[name] || 2"
      [innerHTML]="MARKUP[name] || ''"
    ></svg>
  `,
  styles: [`:host { display: inline-block; width: 1em; height: 1em; flex: none; }`]
})
